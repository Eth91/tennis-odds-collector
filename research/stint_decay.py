...
import os as _os
# research harness: one lead at a time. The archive holds 7 snap_kinds; loading without
# filtering silently blends price times into a single fictitious snapshot.
_SNAP = _os.environ.get("SW_SNAP", "tip")
_ARCH = [("basketball_wnba_odds_hist.sqlite",
          "SELECT player, market, game_date, line, price, book, side FROM props "
          "WHERE sport='basketball_wnba' AND snap_kind='" + _SNAP + "'"),
        ] if _os.environ.get("SW_NEWONLY") else _ARCH_ALL
_seen = set()
for _db, _q in _ARCH:
    try:
...
print(f"[SNAP={_SNAP}] loaded {sum(len(v) for v in LINES.values()):,} quotes")
...
games = [(r["event_id"], r["game_date"]) for r in mem.execute(
    "SELECT DISTINCT event_id, game_date FROM onfloor WHERE game_date >= '" + _os.environ.get('SW_FROM','2025-05-01') + "' "
    "ORDER BY game_date")]
